...
class StripeController extends BaseController
{
    ...
    public function stripeWebhook(Request $request)
    {
        ...
        // Check for subscription payment - get subscription_plan_id from metadata or transaction
        $subscription_plan_id = 0;
        if (!empty($event->data->object->metadata)) {
            $subscription_plan_id = isset($event->data->object->metadata->subscription_plan_id) ? $event->data->object->metadata->subscription_plan_id : 0;
        }
        if ($subscription_plan_id == 0 && isset($transaction) && !empty($transaction)) {
            // Try to get from transaction metadata if available
            $subscription_plan_id = isset($transaction->subscription_plan_id) ? $transaction->subscription_plan_id : 0;
        }
        // Also check if order_id contains subscription pattern
        if ($subscription_plan_id == 0 && !is_numeric($order_id) && strpos($order_id, "subscription") !== false) {
            $temp = explode("-", $order_id);
            ...
            if ($subscription_plan_id > 0 && $user_id > 0) {
                ...
                if ($subscriptionPlan) {
                    ...
                    if (isset($transaction) && !empty($transaction)) {
                        ...
                        $transactionData['message'] = "Subscription payment";
                    ...
                    Log::info("Stripe Webhook | Subscription activated successfully for user: " . $user_id);
                }
