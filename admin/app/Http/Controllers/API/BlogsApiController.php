...
class BlogsApiController extends Controller
{
    ...
    public function getBlogs(Request $request)
    {
        try {
            ...
            $tagIdFilter = $request->input('tag_id', '');
            $include_inactive = (bool) $request->input('include_inactive', false);
            ...
            if (!empty($tagIdFilter)) {
                $query->whereRaw('FIND_IN_SET(?, COALESCE(tags, ""))', [$tagIdFilter]);
            }
            ...
            if (!empty($tagIdFilter)) {
                $totalQuery->whereRaw('FIND_IN_SET(?, COALESCE(tags, ""))', [$tagIdFilter]);
            }
            if (!$include_inactive) {
                $totalQuery->active();
            }
    ...
    public function createBlog(Request $request)
    {
        try {
            ...
            $tagIds = $this->prepareTagIds($request->input('tag_ids'));
            ...
            $blogData = [
                ...
                'meta_description' => $request->meta_description,
                'status' => $request->status,
                'tags' => !empty($tagIds) ? implode(',', $tagIds) : null
            ];
    ...
    {
        try {
            $tags = BlogTag::select('id', 'name')
                          ->orderBy('name', 'asc')
                          ->get();

            return CommonHelper::responseSuccessWithData(__('success'), $tags);
    ...
    public function getPopularBlogTags()
    {
        try {
            ...
            if (empty($tagUsage)) {
                return CommonHelper::responseSuccessWithData(__('success'), []);
            }
            ...
            return CommonHelper::responseSuccessWithData(__('success'), $result);
    ...
    private function appendTagsMetadata($blogs)
    {
        ...
        $allTagIds = $blogs->pluck('tags')
            ->filter()
            ->flatMap(function ($tagString) {
                return $this->parseTagIdsFromString($tagString);
            })
            ->unique()
            ->values()
            ->all();
        ...
        $tagMap = [];
        if (!empty($allTagIds)) {
            $tagMap = BlogTag::whereIn('id', $allTagIds)
                ->pluck('name', 'id')
                ->toArray();
        }

        foreach ($blogs as $blog) {
            $tagIds = $this->parseTagIdsFromString($blog->tags);
            // Only add tag_names, tag_ids is not needed as tags column already contains the IDs
            $blog->tag_names = collect($tagIds)
                ->map(function ($tagId) use ($tagMap) {
    ...
    private function prepareTagIds($tagInput)
    {
        if (empty($tagInput)) {
            return [];
        }
        ...
        foreach ($tagItems as $item) {
            if (is_numeric($item)) {
                ...
                $tagName = $item;
                $blogTag = BlogTag::firstOrCreate(['name' => $tagName]);
                $tagIds[] = $blogTag->id;
            }
        }
