    ...
    public function getProducts(Request $request)
    {
        ...
        try {
            ...
            $products = Product::select(
                ...
                ->selectRaw('MIN(IF(pv.discounted_price > 0, pv.discounted_price, pv.price)) as min_price, MAX(IF(pv.discounted_price > 0, pv.discounted_price, pv.price)) as max_price')->with('ratings');
            ...
            foreach ($products as $row) {
                ...
                for ($k = 0; $k < count($variants); $k++) {
                    ...
                    // If variantData is an object, convert to array to merge product relationship
                    if ($variantData) {
                        $variantDataArray = is_object($variantData) ? $variantData->toArray() : $variantData;
                        
                        // Attach few_quantity_left directly on variant payload
