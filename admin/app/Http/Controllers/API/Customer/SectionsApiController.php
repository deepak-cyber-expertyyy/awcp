...
class SectionsApiController extends Controller
{
    public function getSections(Request $request)
    {
        ...
        if (isset($request->section_id) && $request->section_id != "") {
            ...
            if ($section->product_type != 'custom_products' && $section->product_type != 'recently_visited_products' && !empty($section->product_type)) {
                $product = $sql->get();
                $rows = $tempRow = array();
        ...
        // Filter by product_ids if section_id is set and product_ids are available
        if (isset($request->section_id) && $request->section_id != "" && !empty($product_ids) && $section) {
            // For recently_visited_products, use whereIn for better performance
            if ($section->product_type == 'recently_visited_products' && $product_ids != "0") {
                $product_ids_array = explode(",", $product_ids);
                $sql = $sql->whereIn('p.id', $product_ids_array);
            } elseif ($product_ids != "0") {
                // For other product types, use FIND_IN_SET
                $sql = $sql->whereRaw("FIND_IN_SET(p.id, '$product_ids')");
            } else {
                // If product_ids is "0", return no results
                $sql = $sql->whereRaw("1 = 0");
            }
        }
