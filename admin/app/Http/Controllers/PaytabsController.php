        ...
        try {
            if ($notification['payment_result']['response_status'] == 'A') {
                ...
                if ($explode[0] == 'order') {
                    ...
                    Log::info("Paytabs Callback Subscription: " . print_r($notification, true));
                    ...
                    if ($subscription_plan_id > 0 && $user_id > 0) {
                        ...
                        if ($subscriptionPlan) {
                            ...
                            $transactionData['status'] = Transaction::$statusSuccess;
                            $transactionData['message'] = "Paytabs subscription payment";
                            $transactionData['transaction_date'] = now();
                            $transaction = Transaction::create($transactionData);
                            ...
                            Log::info("Paytabs | Subscription activated successfully for user: " . $user_id);
        ...
        if ($subscription_plan_id > 0 && $user_id > 0) {
            ...
            if ($userSubscription) {
                ...
                $transaction = Transaction::where('user_id', $user_id)
                    ->where('order_id', 0)
                    ->where('type', Transaction::$paymentTypePaytabs)
