...
class SellerController extends BaseController
{
    public function index()
    {
        ...
        $data['weekly_sales'] = Order::select(DB::raw('ROUND(SUM(remaining_final), 2) AS total_sale'), DB::raw('DATE(created_at) AS order_date'))
        ...
        $data['status_order_count'] = OrderStatusList::select(
    ...
    public function getDeliveryBoys(Request $request)
    {
        ...
        $deliveryBoys = DeliveryBoy::select('id', 'name')->where('city_id', $city_id)->where('status', 1);
