...
class CommonHelper
{
        ...
        foreach ($sections as $key => $section) {
            ...
            foreach ($products as $row) {
                ...
                for ($k = 0; $k < count($variants); $k++) {
                    ...
                    $variantData = self::getProductVariant($variants[$k]['id'], $user_id);

                    // Add few_quantity_left to variant data
                    if ($variantData) {
                        $variantDataArray = is_object($variantData) ? $variantData->toArray() : $variantData;
                        // For packet type, use variant-specific few quantity; otherwise use product-level
    ...
    public static function sendReturnRequestNotification($returnRequest)
    {
        ...
        if (isset($returnRequest->user_id) && $returnRequest->user_id != "") {
            $userTokens = UserToken::where('user_id', $returnRequest->user_id)
                ->where('type', 'customer')
                ->get()
                ->pluck('fcm_token', 'platform')
                ->toArray();

            $title = "Your return request #" . $returnRequest->id . " has been " . $status_name;
            $message = "Your return request for order #" . $returnRequest->order_id . " has been updated to " . $status_name . ". Thank you for using " . $app_name . ".";
        ...
        if ($orderItem && isset($orderItem->seller_id) && $orderItem->seller_id != 0) {
            // Get seller's admin_id from Seller model
            $seller = Seller::where('id', $orderItem->seller_id)->first();
            if ($seller && isset($seller->admin_id) && $seller->admin_id != 0) {
                $sellerTokens = AdminToken::where('user_id', $seller->admin_id)
                    ->where('type', 'Seller')
                    ->get()
                    ->pluck('fcm_token', 'platform')
                    ->toArray();

                $title = "Return request #" . $returnRequest->id . " has been " . $status_name;
                $message = "A return request for order #" . $returnRequest->order_id . " has been updated to " . $status_name . ".";

                self::sendNotification($sellerTokens, $title, $message);
            }
        }
        ...
        if (isset($returnRequest->delivery_boy_id) && $returnRequest->delivery_boy_id != 0) {
            // Get delivery boy's admin_id from DeliveryBoy model
            ...
            if ($deliveryBoy && isset($deliveryBoy->admin_id) && $deliveryBoy->admin_id != 0) {
                $deliveryBoyTokens = AdminToken::where('user_id', $deliveryBoy->admin_id)
                    ->where('type', 'Delivery Boy')
                    ->get()
                    ->pluck('fcm_token', 'platform')
                    ->toArray();
                ...
                self::sendNotification($deliveryBoyTokens, $title, $message);
            }
        }
            ...
            foreach ($sellerNotifications as $sellerId => $products) {
                try {
                    ...
                    if (!empty($sellerTokens)) {
                        ...
                        $title = "Low Stock Alert - {$productCount} Product" . ($productCount > 1 ? 's' : '');

                        $message = "Your product" . ($productCount > 1 ? 's have' : ' has') . " low stock. ";
                        if ($productCount == 1) {
                            $message .= "Stock for {$products[0]['product_name']} is {$products[0]['current_stock']} (limit: {$products[0]['low_stock_limit']}).";
                        } else {
                            $message .= "Please check your inventory and restock soon.";
                        }
                    ...
                    Log::error("Error sending low stock notification to seller {$sellerId}: " . $e->getMessage());
                }
            }
        } catch (\Exception $e) {
            Log::error("Error in sendLowStockNotification: " . $e->getMessage());
        }
    }
    ...
    public static function sendNotificationOrderStatus($order, $type = '')
    {
        ...
        // Get all unique seller IDs from order items to notify all sellers whose products are in the order
        $seller_ids = [];
        if ($type == 'order_item_status_update') {
            // For order item status update, use the seller_id from the order item
            if (isset($order->seller_id) && $order->seller_id != 0 && $order->seller_id != "") {
                $seller_ids[] = $order->seller_id;
            }
        } else {
            // For order status update, get all unique seller IDs from all order items
            // Ensure order items are loaded if not already loaded
            if (!isset($order->items) || $order->items === null) {
                $order->load('items');
            }

            if (isset($order->items) && !empty($order->items)) {
                $seller_ids = array_unique(array_column($order->items->toArray(), 'seller_id'));
                // Filter out invalid seller IDs (0 or empty) and reindex array
                $seller_ids = array_values(array_filter($seller_ids, function ($id) {
                    return isset($id) && $id != 0 && $id != "";
                }));
            }
        }
        // Send notification to each seller whose products are in the order
        foreach ($seller_ids as $seller_id) {
            try {
                $seller = Seller::select("sellers.*", "admins.email", "admins.role_id")
                    ...
                    ->first();
                if (!$seller) {
                    Log::warning("Seller not found for ID: {$seller_id} in order #{$order->id}");
                    continue;
                }

                // Check if mail/notification is enabled for this seller
                if (!self::checkOrderMailSendable($seller->admin_id, $order->active_status, 1, "Notification")) {
                    continue;
                }
                // Get FCM tokens for this seller (using admin_id as user_id in AdminToken)
                ...
                self::sendNotification($userTokens, $title, $message, $notification_type);
            } catch (\Exception $e) {
                Log::error("Error sending notification to seller {$seller_id} for order #{$order->id}: " . $e->getMessage());
            }
        }
    ...
    public static function sendNotificationOrderAssignDeliveryBoy($order)
    {
        if (isset($order->delivery_boy_id) && $order->delivery_boy_id != 0 && $order->delivery_boy_id != "") {
            ...
            $deliveryBoyUserId = DeliveryBoy::where('id', $deliveryBoy_id)->value('admin_id');

            $title = "Order  #" . $order->id . " has been assigned to you for delivery.";
            $userTokens = AdminToken::where('user_id', $deliveryBoyUserId)->where('type', 'Delivery Boy')->get()->pluck('fcm_token', 'platform')->toArray();
            $message = "";
            $type = "assign_order";
    ...
    public static function sendNotification($userTokens, $title, $message, $type = '', $type_id = 0, $image = '')
    {
        ...
        $fcmMsg = [
            ...
            'id' => $id ?? '',
            'icon' => $logo,
            'sound' => $type == "new_order" || $type == "assign_order" ?  "order_sound.wav" : "default"
        ];
    ...
    public static function restrictedUrls()
    {
        return array(
            ...
            'seller_commissions.update',
            'seller_commissions.delete',
