               ...
               <div class="col-md-9 text-left mt-1">
                  <div class="position-relative d-inline-block">
                     <input 
                        type="checkbox" 
                        id="is_free_delivery" 
                        v-model="is_free_delivery" 
                        class="form-check-input" 
                        :value="1"
                        :disabled="hasActiveSubscriptionPlans"
                        v-b-tooltip.hover
                        :title="hasActiveSubscriptionPlans ? __('free_delivery_disabled_active_subscription_plans_tooltip') : ''"
                     >
                     <label for="is_free_delivery" class="form-check-label ml-2" :class="{ 'text-muted': hasActiveSubscriptionPlans }">
                        {{ __('enable_free_delivery') }}
                     </label>
                  </div>
                  <div v-if="hasActiveSubscriptionPlans" class="alert alert-success mt-2 mb-0" role="alert">
                     <small>
                        <i class="fa fa-info-circle"></i>
                        <strong>{{ __('note') }}:</strong> {{ __('free_delivery_disabled_active_subscription_plans_message') }}
